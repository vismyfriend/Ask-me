...
function showQuestionsManual() {
    ...
    manual1.textContent = "Прочитай сперва молча, про себя, не вслух. Постарайся запомнить вопрос. Не обязательно знать перевод."
    manual2.textContent = "Если не можете понять что написано или хотите убедиться - нажмите на кнопку 'перевод'"
    manual3.textContent = "Набери воздуха в легкие и уверенно, с вопросительной интоннацией задай этот вопрос вслух. Старайся не читать по словам, а произнести естественно и целиком всё."
    hint.textContent = "Всегда можно задать дополнительные вопросы, если захочется - например : What / When / How often / Who / Where ... ? в тзаписывая мы развиваем и тренируем память.)"
...
function showCheckManual() {
    manual1.textContent = "Вы "
    manual2.textContent = "Куда-то"
    manual3.textContent = "Не туда нажали)"
    hint.textContent = "When / How often / Who / Where?"
}
